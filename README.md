...
I emulated the Azure service connection using GitHub Environments and secrets: the workflow reads environment secrets and conditionally performs a demo deploy or a production deploy, keeping approvals and branch protection intact — so the pipeline’s control flow and audit trail match production even without a cloud subscription.
